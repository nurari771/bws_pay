<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['high_search'])) {
		...
		if (!empty($member_id) && empty($position_id) && empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.member_id = '$member_id' ORDER BY ot.ot_id " ;
		} else if (empty($member_id) && !empty($position_id) && empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.position_id = '$position_id' ORDER BY ot.ot_id " ;
		} else if (empty($member_id) && empty($position_id) && !empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id= member.member_id inner join position on member.position_id = position.position_id WHERE ot.makeday_ot = '$makeday_ot' ORDER BY ot.ot_id " ;
		} else if (!empty($member_id) && !empty($position_id) && empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.member_id = '$member_id' AND member.position_id = '$position_id' ORDER BY ot.ot_id " ;
		} else if (empty($member_id) && !empty($position_id) && !empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.position_id = '$position_id' AND ot.makeday_ot = '$makeday_ot' ORDER BY ot.ot_id " ;
		} else if (!empty($member_id) && empty($position_id) && !empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.member_id = '$member_id' AND ot.makeday_ot = '$makeday_ot' ORDER BY ot.ot_id " ;
		} else if (!empty($member_id) && !empty($position_id) && !empty($makeday_ot)) {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.member_id = '$member_id' AND member.position_id = '$position_id' AND ot.makeday_ot = '$makeday_ot' ORDER BY ot.ot_id " ;
		} else {
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id inner join position on member.position_id = position.position_id ORDER BY ot.ot_id " ;
		} 
	}
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['month'])) {
		...
		if ($ot_serach == 0) {
			...
			$ot = "SELECT * FROM ot inner join member on ot.member_id = member.member_id 
		inner join position on member.position_id = position.position_id	WHERE MONTH(ot.makeday_ot) = '$ot_serach' ORDER BY ot.ot_id " ;
		}
	}
				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									<a href="index.html" class="logo"><strong>รายงานรวมรวมค่าOT</strong></a>
									<ul class="icons">
 ...
 <tr>
   <div class="form-row">
	<td align="right"><label for="m_report">เดือน: </label></td>
	 ...
	 <form method="post">
	  	<select id="ot_serach" class="form-control" name="ot_serach">
							...
							<div class="form-row">
								...
								<div class="form-row">
							  	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label for="makeday_day">ปฏิทิน: </label>&nbsp;&nbsp;&nbsp;
										<div class="form-group col-md-3">
									    <input type="date" class="form-control" id="makeday_ot" name="makeday_ot" placeholder="วัน/เดือน/ปี">
		...
		<table class="alt">
			<thead>
				...
				<?php
									...
									<a href="admin_excelot.php?month=<?php echo $ot_serach; ?>&position_id=<?php echo $position_id; ?>&member_id=<?php echo $member_id; ?>&makeday_ot=<?php echo $makeday_ot ?>" class="button primary">Excel</a>
								</section>
				...
				<!-- Sidebar -->
					<div id="sidebar">
						<div class="inner">
							...
							<!-- Footer                                                                                                                                             พงศธร รับผิดชอบ backendหน้านี้
							-->
								<footer id="footer">
								</footer>
