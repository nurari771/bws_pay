<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['high_search'])) {
		...
		if (!empty($member_id) && empty($position_id) && empty($makeday_travel)) {
			...
			$travel = "SELECT * FROM travel inner join member on travel.member_id = member.member_id inner join position on member.position_id = position.position_id WHERE member.position_id = '$position_id' AND travel.makeday_trsvel = '$makeday_travel' ORDER BY travel.travel_id " ;
		} else if (!empty($member_id) && empty($position_id) && !empty($makeday_travel)) {
